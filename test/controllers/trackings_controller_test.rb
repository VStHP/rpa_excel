...
class TrackingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create tracking" do
    assert_difference('Tracking.count') do
      post trackings_url, params: { tracking: { check_in: @tracking.check_in, check_out: @tracking.check_out, date: @tracking.date, status: @tracking.status, user_name: @tracking.user_name, working_time: @tracking.working_time } }
    end
  ...
  test "should update tracking" do
    patch tracking_url(@tracking), params: { tracking: { check_in: @tracking.check_in, check_out: @tracking.check_out, date: @tracking.date, status: @tracking.status, user_name: @tracking.user_name, working_time: @tracking.working_time } }
