class TrackingsController < ApplicationController
  ...
  def export_xlsx
    respond_to do |f|
      f.xlsx do
        template_file = Template.find_by(attachable_type: 'Tracking')
        if template_file
          write_and_download(template_file.file, 'tracking')
          if @message_danger
            flash[:danger] = @message_danger
            redirect_to root_path
          end
        else
          flash[:danger] = "Missing file tracking template, let's import!"
          redirect_to root_path
        end
      end
      f.html do
        flash[:danger] = 'Wrong URL'
        redirect_to root_path
      end
    end
  end
  ...
  private
    ...
    def tracking_params
      params.require(:tracking).permit(:user_name, :check_in, :check_out)
    end
end
